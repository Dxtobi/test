...
<aside class=" flex flex-col p-4 w-[20%] border-l">
    {#if active_element?.component}
        <div class="flex justify-between items-start">
            <h1 class=" font-bold text-xl uppercase text-600 mb-4">{active_element.component}</h1>
            <button onclick={() => all_comps.element_removed(active_element.id)}
                ><iconify-icon icon="ic:round-delete" class=" text-2xl"></iconify-icon></button
            >
        </div>
    {/if}
    <div>
        {#if active_element.styles}
            ...
            <div class="flex flex-col gap-2 capitalize">
                ...
                {#each stylesArray as [key, value], i}
                    ...
                    {:else if key === 'border_radius' || key === 'border_width' || key === 'padding'}
